// https://github.com/cyclejs/cyclejs/blob/313762d9beb87e099bb36511875c42bd7e1ff980/run/src/internals.ts#L14
...
export function callDrivers<D extends Drivers>(
  ...
  for (const name in drivers) {
    if (drivers.hasOwnProperty(name)) {
      (sources as any)[name] = (drivers[name] as any)(sinkProxies[name], name);
...
export function replicateMany<Si extends any>(
  ...
  const sinkNames = Object.keys(sinks).filter(
